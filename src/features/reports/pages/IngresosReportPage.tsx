...
import { fetchIngresos } from '../api/reports.api';
...
import {
  DEFAULT_INGRESOS,
  useIngresosFiltersFromUrl,
  useIngresosOptions,
} from '../hooks/useReports';
import type { IngresoRecord, IngresosResponse } from '../types/reports.types';
...
const columns: ColumnDef<IngresoRecord>[] = [
  ...
  },
  {
    ...
    className: styles.truncate,
    render: (r) => (
      <span title={r.materia_prima ?? undefined}>{truncateText(r.materia_prima, 20)}</span>
    ),
  },
  {
    key: 'proveedor',
    ...
    className: styles.truncate,
    render: (r) => (
      <span title={r.proveedor ?? undefined}>{truncateText(r.proveedor, 20)}</span>
    ),
  },
  {
    key: 'planta',
...
export function IngresosReportPage() {
  ...
  const options = useIngresosOptions(apiAuth);
  const [filters, setFilters] = useIngresosFiltersFromUrl();

  const [response, setResponse] = useState<IngresosResponse | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [selected, setSelected] = useState<IngresoRecord | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await fetchIngresos(apiAuth, filters);
      setResponse(result);
    } catch (err) {
  ...
  async function handlePrint() {
    ...
    try {
      await ticket.openForReprint('Ingreso', selected.no_tiquete);
    } catch (err) {
      if (err instanceof ApiUnauthorizedError) {
        navigate('/login', { replace: true });
        return;
  ...
  return (
    <ReportsLayout title="Ingresos">
      <ReportFilters
        type="ingresos"
        filters={filters}
        onChange={setFilters}
        onClear={() => setFilters(DEFAULT_INGRESOS)}
        options={options}
      />
      ...
      <ReportTable
        ...
        emptyMessage="Aún no hay registros de ingresos."
        emptyWithFiltersMessage="No se encontraron ingresos con los filtros aplicados."
        errorMessage={error ?? undefined}
        hasActiveFilters={activeFilters}
        onRetry={load}
        onClearFilters={() => setFilters(DEFAULT_INGRESOS)}
